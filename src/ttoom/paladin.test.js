...
test('calculateHealingHelper for SacredShield', () => {
    ...
    expect(Math.floor(amount)).toBe(2812); //double checked againstr currelius' sheet

    // divine plea does not affect sacred shield
    let amountWithPlea = player.calculateHealing('SACRED_SHIELD', false, true);
    expect(Math.floor(amountWithPlea)).toBe(2812); //double checked againstr currelius' sheet
});
